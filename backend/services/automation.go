...
func (a *AutomationService) buildReport(job *AutomationJob, rr *ResearchReport, confidence, contentScore float64, safetyPassed bool, publishDecision, rejectReason string) *AutomationReport {
	...
	if rr != nil {
		...
		report.KeyFindings = rr.KeyFindings
...
// createBlogPost generates a Bloomberg-level editorial blog post from the research report.
func (a *AutomationService) createBlogPost(job *AutomationJob, report *AutomationReport) BlogPost {
	...
	// Editorial headline — no "Research Report" framing
	content.WriteString(fmt.Sprintf("# %s\n\n", report.Title))

	// Lead with a bold hook/summary — not a dry abstract
	content.WriteString(fmt.Sprintf("> %s\n\n", report.Summary))

	// Confidence & source badge
	content.WriteString(fmt.Sprintf("**Confidence:** %.0f%% · **%d verified sources** · *Published %s*\n\n---\n\n",
		report.Confidence*100, len(report.Sources), time.Now().Format("January 2, 2006")))

	// Key findings as a scannable callout box
	...
	// Sections — each with editorial formatting
	for _, section := range report.Sections {
		content.WriteString(fmt.Sprintf("## %s\n\n%s\n\n", section.Title, section.Content))
	}

	// Sources as references, not a data dump
	if len(report.Sources) > 0 {
		content.WriteString("---\n\n## References & Sources\n\n")
		for i, src := range report.Sources {
			if i >= 10 {
				content.WriteString(fmt.Sprintf("*... and %d additional sources reviewed*\n", len(report.Sources)-10))
				break
			}
			if src.URL != "" {
				content.WriteString(fmt.Sprintf("- [%s](%s) — *%s*\n", src.Title, src.URL, src.Agent))
			} else {
	...
	// Editorial footer
	content.WriteString("\n---\n\n*This analysis was generated by Carelwave Media's AI Research Lab using multi-agent verification. ")
	content.WriteString("Data points are cross-referenced across multiple sources for accuracy.*\n")
